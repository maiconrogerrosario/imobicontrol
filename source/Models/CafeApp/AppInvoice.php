...
class AppInvoice extends Model
{
     ...
     * @return AppInvoice|null
     */
    public function customerInvoice(User $user, array $data, string $wallet, string $value): ?AppInvoice
    {
		...
		$wallet = (new AppWallet())->find("application_id = :application_id AND  wallet = :wallet AND customer_id = :customer_id",
            "application_id={$user->application_id}&wallet={$wallet}&customer_id={$data["customer_id"]}")->fetch();
     ...
     * @return AppInvoice|null
     */
    public function ownerTransfer(User $user, array $data, string $wallet): ?AppInvoice
    {
